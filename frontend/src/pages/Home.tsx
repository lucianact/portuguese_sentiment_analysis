...
function isGibberish(text: string) {
  ...
  if (letters.length < 10) return true; // too short

  const vowelCount = letters.filter((c) => vowels.includes(c)).length;
  const vowelRatio = vowelCount / letters.length;

  // stricter threshold for longer texts
...
export default function Home() {            
  ...
  // NEW: loading state that appears only if the request hasn't returned after a short delay
  const [isLoading, setIsLoading] = useState(false);               // spinner / "waking up" message
  const loadingDelayRef = useRef<number | null>(null);             // timer id for delayed spinner
  ...
  // Track whether we've hit the API at least once this session (for smarter loading copy)
  ...
  useEffect(() => {
    return () => {
      ...
      if (loadingDelayRef.current) window.clearTimeout(loadingDelayRef.current);
  ...
  const sendFeedback = async (correctLabel: number) => {
    ...
    if (ok) {
      ...
      // Consider API "woken" after any successful round-trip
      setApiWokenUp(true);
  ...
  /* ----------------------------------------
     ...
     - NEW: shows spinner & "waking up" message only if slow
  ---------------------------------------- */
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // NEW: delayed spinner/message — only show if request is still pending after DELAY_MS
    const DELAY_MS = 700; // tweak as you like (500–1000ms is typical)
    if (loadingDelayRef.current) window.clearTimeout(loadingDelayRef.current);
    loadingDelayRef.current = window.setTimeout(() => {
      ...
      setStatusMessage(
        apiWokenUp
          ? "⏳ Working on it…"
          : "⏳ Waking up the API… this can take a few seconds."
      );
      setStatusType("default");
    }, DELAY_MS);
    ...
    // Clear delayed spinner timer *first*
    if (loadingDelayRef.current) {
      window.clearTimeout(loadingDelayRef.current);
      loadingDelayRef.current = null;
    }

    // Hide spinner if it had appeared
    setIsLoading(false);
    ...
    // Mark API as "woken" after a successful predict
    setApiWokenUp(true);
  ...
  return (
    <main className="app" role="main">
      ...
      {/* Spinner (appears only if request is slow due to the delayed timer) */}
      {isLoading && (
